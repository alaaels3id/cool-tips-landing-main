...
const TopicsSection = () => {
  ...
  return (
    <section className="py-40 relative bg-background overflow-hidden border-t-8 border-foreground">
      ...
      <div className="absolute top-0 right-[-5%] w-64 h-64 bg-accent/20 rounded-full blur-[80px] animate-bounce-slow" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 animate-reveal" style={{ animationDelay: '0.2s' }}>
          {topics.map((topic, index) => (
            <div 
              ...
              `}
            >
              <div className={`w-20 h-20 rounded-[1.5rem] border-4 border-foreground flex items-center justify-center p-4 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6
